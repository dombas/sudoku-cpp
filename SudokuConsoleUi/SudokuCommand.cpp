...
namespace Sudoku {
	void SudokuCommand::parse_input()
	{
		...
		case ParseState::no_column:
			...
			string_stream << "1 - indicator of column, any from " << column_symbols << endl;
		...
		case ParseState::no_value:
			...
			string_stream << "Clearing field at row " << to_string(row).at(0) << ", column " << to_string(column).at(0);
			message = string_stream.str();
		...
		case ParseState::all_found:
			...
			string_stream << "Setting field at row " << to_string(row).at(0) << ", column " << to_string(column).at(0) << " to " << to_string(value).at(0);
			message = string_stream.str();
